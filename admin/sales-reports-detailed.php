...
<html lang="en">
<head>
<title>Flour Milling System|| Sales Report Details</title>
...
$ret=mysqli_query($con,"select month(OrderTime) as lmonth,year(OrderTime) as lyear,sum(SubTotal) as totalitmprice from payment join tblorder on tblorder.Ordernumber=payment.OrderNumber join product on product.ID=payment.PId where date(tblorder.OrderTime) between '$fdate' and '$tdate' and tblorder.OrderFinalStatus='$fstatus'  group by lmonth,lyear");
$cnt=1;
...
<?php
$ret=mysqli_query($con,"select year(OrderTime) as lyear,sum(SubTotal) as totalitmprice from payment join tblorder on tblorder.Ordernumber=payment.OrderNumber join product on product.ID=payment.PId where year(tblorder.OrderTime) between '$fdate' and '$tdate' group by lyear");
$cnt=1;
while ($row=mysqli_fetch_array($ret)) {
