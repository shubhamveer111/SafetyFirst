...
<head>
    <title>SafetyFirst||My Orders</title>
    <!-- Vendor CSS Files -->
...
$ret=mysqli_query($con,"select OrderTime,OrderFinalStatus from tblorder where UserId='$userid' and Ordernumber='$oid'");
while($result=mysqli_fetch_array($ret)) {
?>

<p style="color:#000"><b>Order #</b><?php echo $oid?></p>
<p style="color:#000"><b>Order Date : </b><?php echo $od=$result['OrderTime'];?></p>
<p style="color:#000"><b>Order Status :</b> <?php if($result['OrderFinalStatus']=="Wait for confirmation"){
    echo "Order Not Accpet Yet";
} else {
echo $result['OrderFinalStatus'];
}?> &nbsp;
<a href="javascript:void(0);" onClick="popUpWindow('trackorder.php?oid=<?php echo $oid;?>');" title="Track order" style="color:#000" class="btn btn--box btn--small btn--blue btn--uppercase btn--weight">Order Status Details</a></p>
...
<p>
 <a href="javascript:void(0);" onClick="popUpWindow('invoice.php?oid=<?php echo $oid;?>&&odate=<?php echo $od;?>');" title="Order Invoice" style="color:#000" class="btn btn--box btn--small btn--blue btn--uppercase btn--weight">  Invoice</a></p>
                    <?php 
                                $userid= $_SESSION['msmsuid'];
 $query=mysqli_query($con,"select product.Image1,product.ProductName,product.Description,product.Price,payment.PId,payment.OrderNumber,payment.Quantity,payment.SubTotal,payment.PaymentType from payment join product on product.ID=payment.PId where payment.UserId='$userid' and payment.OrderNumber=$oid");
