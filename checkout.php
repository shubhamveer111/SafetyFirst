...
if(isset($_GET['pids'])){
    $pids_json = $_GET['pids'];
    $pqtys_json = $_GET['pqtys'];
    $names_json = $_GET['names'];
    $psubtotal_json = $_GET['psubtotal'];

    $pids = json_decode($pids_json,true);
    $pqtys = json_decode($pqtys_json,true);
    $names = json_decode($names_json,true);
    $psubtotal = json_decode($psubtotal_json,true);
}
...
if($_POST["method"]=="cod"){
    for($i=0;$i<count($pids);$i++){
      ...
      //  echo '<script>alert("Your order placed for item "'+$i+'" successfully. Order number is "+"'.$orderno.'")</script>';
        ...
        echo "<script>window.location.href='order-placed-msg.php'</script>";
        $query="insert into payment (UserId,PId,IsOrderPlaced,OrderNumber,Quantity,SubTotal,PaymentType,cardName,cardNumber,expMonth,expYear,CVV) values($userid,$temp3,1,$orderno,$temp1,$temp2,'$payment','$cardname','$cardnu','$expMonth','$expYear','$cvv')";
        ...
        $query5="insert into tblorder(UserId,Ordernumber,Flatnobuldngno,StreetName,Area,Landmark,City,Zipcode) values('$userid','$orderno','$fnaobno','$street','$area','$lndmark','$city','$zipcode');";    
...
<html lang="zxx">
<head>
    <title>SafetyFirst||Checkout Page</title>
    <!-- Vendor CSS Files -->
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-box__single-group">
                                    ...
                                    <input type="text" name="streename" required="true" pattern="[a-zA-Z'-'\s]*" placeholder="Street Name" class="form-control" required="true">
                                </div>
                            </div>
                           ...
                           <div class="col-md-6">
                                <div class="form-box__single-group">
                                    ...
                                    <input type="text" id="zipcode" class="form-control" name="zipcode"  maxlength="6" pattern="[0-9]{6}" required="true">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-box__single-group">
                                    <label for="form-zipcode"> Landmark if any</label>
                                    <input type="text" id="zipcode"  pattern="[a-zA-Z'-'\s]*" class="form-control" name="landmark">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-box__single-group">
                                    <label for="form-zipcode"> Town / City *</label>
                                    <input type="text" name="city" pattern="[a-zA-Z'-'\s]*"  placeholder="City" class="form-control" required="true">
                                </div>
                            </div>
                        ...
                        <div class="your-order-box gray-bg m-t-40 m-b-30">
                            <div class="your-order-product-info">
                                <div class="your-order-top d-flex justify-content-between">
                                 ...
                                 <?php 
$userid= $_SESSION['msmsuid'];
$query=mysqli_query($con,"select product.Image1,product.ProductName,product.CategoryName,product.Price,payment.PId,payment.ID from payment join product on product.ID=payment.PId where payment.UserId='$userid' and payment.IsOrderPlaced is null");
...
?>
                                <ul class="your-order-middle">
                                    <li class="d-flex justify-content-between">
                                        ...
                                        <span class="your-order-middle-left"><?php echo $pqtys[$cnt_qty];?></span>
                                        ...
                                        <div class="panel-group" id="accordion">
                                            ...
                                            <div class="panel payment-accordion">
                                                <div class="panel-heading" id="method-three">
                                                    ...
                                                    <Label style="font-size:25px;"><u>Payment Method:</u></label>                                 
                                                    <div>
                                                     <input type="radio" checked="checked" name="method"  id="cod" value="cod" hidden> <br>
                                                           
                                                    <label > Cash On Delivery
                                                    <input  type="radio" checked="checked" name="payment"  id="cash" value="COD" > <br>
                                                    <label>Credit/Debit
                                                    ...
                                                    <input type="text" name="cardname"  pattern="[a-zA-Z'-'\s]*" placeholder="Enter Name">
                                                   
                                                    <label>Card Number:</label>
                                                    <input type="text" name="cardnu" maxlength="16" pattern="[0-9]{16}" placeholder="**** **** **** ****"  maxlength=16>

                                                    <label>Exp Month:</label>
                                                    <input type="text" name="expMonth" maxlength="2" pattern="[0-9]{2}" placeholder="Enter Exp Month"  maxlength=2>

                                                    <label>Exp Year:</label>
                                                    <input type="text" name="expYear" maxlength="4" pattern="[0-9]{4}" placeholder="****"  maxlength=4><br>

                                                    <label>CVV:</label><br>
                                                    <input type="text" name="cvv" maxlength="3" pattern="[0-9]{3}" placeholder="***"  maxlength=3>
